<template>
  ...
  >
    <div v-if="mostraButtonNuovaProposta" class="w-full p-4">
      <Button
        class="w-full"
        label="Fai la tua proposta"
        @click="mostraDialogFormProposta = true"
      />
      <Dialog
        v-model:visible="mostraDialogFormProposta"
        modal
        header="FORM NUOVA PROPOSTA"
        :style="{ width: '30rem' }"
        :breakpoints="{ '1199px': '75vw', '575px': '90vw' }"
      >
        <FormNuovaProposta
          :prezzo="prezzoImmobile"
    ...
    <div
      v-else
      class="w-full p-4 flex flex-col gap-2 items-center justify-center"
    >
      <span class="text-xl font-bold"
        >Accedi per poter fare la tua proposta</span
      >
      <Button
        class="w-full"
        label="Accedi"
        @click="visibleLoginDialog = true"
        @close="visibleLoginDialog = false"
      />
    </div>
            ...
            <DataTable :value="filterProposteInTrattativa(props.proposte)">
              ...
              <Column field="prezzoProposta" header="Proposta" sortable>
                <template #body="slotProps">
                  <span
                    :class="{
                      'line-through': slotProps.data.controproposta !== 0,
                    }"
                  >
...
const setPrezzoImmobile = () => {
  if (props.contratto.tipoContratto === "AFFITTO") {
    prezzoImmobile.value =
      props.contratto.contrattoAffittoResponse.prezzoAffitto;
  } else {
    prezzoImmobile.value =
      props.contratto.contrattoVenditaResponse.prezzoVendita;
  }
};
