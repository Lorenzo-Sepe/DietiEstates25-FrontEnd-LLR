    ...
    <Dialog v-model:visible="impostazioniSottoscrizineVisibe" modal :style="{ width: 'auto' }"
        ...
        <template #container="{ closeCallback }">
            <div class="flex flex-col p-4">
                ...
                <ImpostazioniSottoscrizioni :categorie="props.categorie" @chiudiImpostazioni="closeCallback"
                    @modificaSottoscrizioni="emit('modificaSottoscrizioni')" />
            </div>
        </template>
    </Dialog>

    <Select v-model="selectedCategoria" :options="nomiCategorie" optionLabel="name" placeholder="Tutte le categorie"
...
const clickCategoria = () => {
    ...
    router.push({
        path: route.path,
        query: { ...route.query, nome: selectedCategoria.value.name, id: selectedCategoria.value.id, pagina: 0 }
    });
}
...
const updateCategorie = () => {
    ...
    props.categorie.forEach(categoria => {
        nomiCategorie.value.push({ name: categoria.nomeCategoria, id: categoria.idCategoria });
    });
