<template>
    ...
    <Dialog v-model:visible="dialogNessunaModifica" modal header="AVVISO" :style="{ width: 'auto' }"
        :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
        <p>Non hai effettuato nessuna modifica rispetto alle attuali sottoscrizioni</p>
        <Button label="OK" @click="dialogNessunaModifica = false" />
    </Dialog>

    <Dialog v-model:visible="dialogConfermaModifiche" modal :style="{ width: 'auto' }"
        :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
        <template #container="{ closeCallback }">
            <div class="flex flex-col p-4">
                <h2 class="text-lg font-bold">ATTENZIONE</h2>
                <p>Non riceverai ulteriori notifiche relative alle categorie disattivate dal momento che confermi.</p>
                <div class="flex justify-end gap-2 my-4">
                    <Button label="Annulla" severity="secondary" raised @click="closeCallback" />
                    <Button label="Conferma" severity="success" raised
                        @click="() => { emit('modificaSottoscrizioni'); closeCallback(); emit('chiudiImpostazioni') }" />
                </div>
            </div>
        </template>
    </Dialog>
    ...
    <div class="flex flex-col">
        <div v-for="(categoria, index) in props.categorie" class="flex justify-between items-center p-1 gap-4">
            <label>{{ categoria.nomeCategoria }}</label>
            <ToggleSwitch v-model="checked[categoria.idCategoria]"
                @click="toggleSottoscrizione(categoria.idCategoria)" />
        </div>
        <div class="flex justify-end gap-2 my-4">
            <Button label="Annulla" severity="secondary" raised @click="clickAnnulla" />
            <Button label="Conferma" severity="success" raised @click="clickConferma" />
        </div>
...
onMounted(() => {
    ...
    sottoscrizioniIniziziali.value = props.categorie.map(categoria => ({ ...categoria }));
