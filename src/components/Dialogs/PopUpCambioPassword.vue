<template>
  <div v-if="stato === 'caricamento'" class="text-center p-6">
    ...
    <p class="text-gray-500">Verifica in corso prima di procedere al cambio password.</p>
  </div>

  <div v-else-if="stato === 'inserimento'" class="text-center p-6">
    <p class="text-xl font-semibold mb-2">Cambio password</p>
    <p class="text-gray-600 mt-2">Inserisci la tua password attuale e la nuova password.</p>

    <!-- Modulo aggiornato per richiedere password attuale -->
    <ModuloPassword
      @submit="cambiaPassword"
      cambioPassword
    />
  </div>
  ...
  <div v-else-if="stato === 'successo'" class="text-center p-6 text-green-600">
    ...
    <p class="text-gray-700 mt-2">La tua password è stata cambiata con successo.</p>
  </div>
...
const cambiaPassword = async ({ passwordAttuale, password, confermaPassword }) => {
  errore.value = null
  ...
  try {
    await AuthService.changePassword(passwordAttuale,password,confermaPassword);
    ...
    if (e.response?.data?.message?.includes("La password inserita non corrisponde")) {
      errore.value = 'La password attuale inserita non è corretta.'
    } else {
      errore.value = 'Si è verificato un errore durante il cambio password. riprovare più tardi.'
    }
    stato.value = 'errore'
  }
}
