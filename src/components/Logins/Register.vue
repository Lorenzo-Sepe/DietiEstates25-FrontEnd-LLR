<template>
  <div fluid class="card p-4 fluid">
    ...
    >
      ...
      />
      <Message
        v-if="$form.nomeVisualizzato?.invalid"
        severity="error"
        size="small"
        >{{ $form.nomeVisualizzato.error?.message }}</Message
      >
...
const resolver = ({ values }) => {
  ...
  if (
    !values.nomeVisualizzato ||
    values.nomeVisualizzato.length < 3 ||
    values.nomeVisualizzato.length > 20
  ) {
    errors.nomeVisualizzato = [
      { message: "Nome deve avere tra 3 e 20 caratteri." },
    ];
  }
