<template>
    <div fluid class="card p-4 fluid">
        <Form v-slot="$form" :initialValues="initialValues" :resolver="resolver" @submit="handleRegister" class="flex flex-col gap-4 ">
            ...
            />
            <Message v-if="$form.nome?.invalid" severity="error" size="small">{{ $form.nome.error?.message }}</Message>
            ...
            />
            <Message v-if="$form.cognome?.invalid" severity="error" size="small">{{ $form.cognome.error?.message }}</Message>
...
const resolver = ({ values }) => {
    ...
    if (!values.cognome || values.cognome.length < 3 || values.cognome.length > 20) {
