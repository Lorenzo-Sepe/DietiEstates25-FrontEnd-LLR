  ...
  <div class="">
    ...
    <div class="flex flex-col gap-4">
      ...
      <div class="flex flex-col">
        ...
        <InputText id="titolo" v-model="annuncio.titolo" :invalid="errori.titolo.invalid"
          @input="verificaDati('titolo')" @blur="verificaDati('titolo')" class="!border-1 !border-black !rounded-lg !p-2" />
      ...
      <div class="flex flex-col">
        ...
        <Textarea id="descrizione" v-model="annuncio.descrizione" rows="4" :invalid="errori.descrizione.invalid"
          @input="verificaDati('descrizione')" @blur="verificaDati('descrizione')" class="!border-1 !border-black !rounded-lg !p-2" />
        <Message v-if="errori.descrizione.invalid" severity="error" variant="simple" size="small">{{
          errori.descrizione.messaggio }}</Message>
      </div>

      <div class="flex flex-col">
        <label for="tipoContratto" class="font-semibold">Tipo di Contratto</label>
        <Select id="tipoContratto" v-model="annuncio.contratto.tipoDiContratto" :options="opzioniContratto"
          optionLabel="label" optionValue="value" placeholder="Seleziona il contratto"
          :invalid="errori.contratto.invalid" @blur="verificaDati('contratto')" @change="verificaDati('contratto')"
          class="!border-1 !border-black !rounded-lg !p-2" />
        <Message v-if="errori.contratto.invalid" severity="error" variant="simple" size="small">{{
          errori.contratto.messaggio }}</Message>
      </div>

      <div class="flex flex-col">
        <label for="tipologiaImmobile" class="font-semibold">Tipologia di Immobile</label>
        <Select id="tipologiaImmobile" v-model="annuncio.immobile.tipologiaImmobile" :options="opzioniTipologia"
          optionLabel="label" optionValue="value" placeholder="Seleziona la tipologia"
          :invalid="errori.tipologia.invalid" @blur="verificaDati('tipologia')" @change="verificaDati('tipologia')"
          class="!border-1 !border-black !rounded-lg !p-2" />
        <Message v-if="errori.tipologia.invalid" severity="error" variant="simple" size="small">{{
          errori.tipologia.messaggio }}</Message>
      </div>
    </div>
