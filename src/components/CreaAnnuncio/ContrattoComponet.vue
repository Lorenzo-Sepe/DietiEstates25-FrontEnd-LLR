  ...
  if (props.contratto.tipoDiContratto === "AFFITTO") {
    ...
    if (campo === "tempoMinimo") {
      ...
      errori.tempoMinimo.messaggio =
        tempoMinimo <= 0 ? "Il tempo minimo deve essere maggiore di zero" : "";
    }
    if (campo === "tempoMassimo") {
      ...
      if (tempoMassimo <= 0) {
        ...
        errori.tempoMassimo.messaggio =
          "Il tempo massimo deve essere maggiore di zero";
      } else {
...
const validaCampi = () => {
  ...
  if (props.contratto.tipoDiContratto === "AFFITTO") {
    ["prezzo", "caparra", "tempoMinimo", "tempoMassimo"].forEach((campo) =>
      verificaDati(campo),
    );
  } else if (props.contratto.tipoDiContratto === "VENDITA") {
...
watch(
  () => props.contratto.tipoDiContratto,
  () => {
...
<template>
  <div class="border border-black p-4 space-y-6 my-4">
    ...
    <div class="mb-6">
      <label for="tipoContratto" class="block font-semibold mb-2"
    ...
    <div
      v-if="contratto.tipoDiContratto"
      class="border-1 border-black p-4 rounded"
    >
      <!-- Sezione per il contratto in AFFITTO -->
      <div
        v-if="contratto.tipoDiContratto === 'AFFITTO'"
        class="flex flex-col md:flex-row gap-4 justify-center items-centerr"
      >
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="flex flex-col">
            ...
            <div class="min-h-[24px]">
              <Message
                v-if="errori.tempoMinimo.invalid"
                severity="error"
                variant="simple"
                size="small"
              >
          ...
          <div class="flex flex-col">
            ...
            <div class="min-h-[24px]">
              <Message
                v-if="errori.tempoMassimo.invalid"
                severity="error"
                variant="simple"
                size="small"
              >
      ...
      <div
        v-if="contratto.tipoDiContratto === 'VENDITA'"
