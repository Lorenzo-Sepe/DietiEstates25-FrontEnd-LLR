...
const etichettaOpzione = (comune) =>
  `${comune.denominazione_ita} (${comune.sigla_provincia}) CAP:${comune.cap}`;
...
if (props.comune.cap) {
  const record = listaComuni.value.find(
    (comune) => comune.cap === props.comune.cap,
  );
  if (record) {
...
const aggiornaIndirizzo = (capSelezionatoVal) => {
  ...
  const record = listaComuni.value.find(
    (comune) =>
      comune.cap === capSelezionatoVal.cap || comune.cap === capSelezionatoVal,
  );
