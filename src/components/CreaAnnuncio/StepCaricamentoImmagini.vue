<template>
  ...
  <div class="contenitore-card">
    ...
    <div class="barra-superiore">
      ...
      <Button @click="apriSelettore" icon="pi pi-image" label="Aggiungi immagini" outlined class="pulsante" />
    ...
    <div v-if="annuncio.immobile.immagini.length" class="griglia-immagini">
      <div v-for="(img, indice) in annuncio.immobile.immagini" :key="indice" class="carta-immagine">
...
const convertiImmaginiEsistenti = async () => {
  ...
  for (const img of props.annuncio.immobile.immagini) {
    ...
    if (!img.file || (typeof img.file === 'object' && Object.keys(img.file).length === 0)) {
      ...
      if (img.urlImmagineEsistente.startsWith('data:')) {
        ...
        creaFileDaUrl(img.urlImmagineEsistente, 'immagine.jpg').then((file) => {
          img.file = file; // Assegna il file creato all'immagine
          img.loading = false; // Rimuovi il flag di caricamento
        }).catch((error) => {
          console.error('Errore nella conversione dell\'immagine da base64:', error);
        ...
        creaFileDaUrlRemoto(img.urlImmagineEsistente, 'immagine.jpg').then((file) => {
          img.file = file; // Assegna il file creato all'immagine
          img.loading = false; // Rimuovi il flag di caricamento
        }).catch((error) => {
          console.error('Errore nella conversione dell\'immagine da URL remoto:', error);
          img.loading = false; // Rimuovi il flag di caricamento in caso di errore
        });
      }
    } else {
