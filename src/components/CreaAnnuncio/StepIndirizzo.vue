    ...
    <div class="flex flex-col md:flex-row gap-4">
      <div class="flex flex-col w-full md:w-1/2">
        <div class="flex flex-col">
          ...
          <InputText
            id="via"
            v-model="annuncio.immobile.indirizzo.via"
            :invalid="errori.via.invalid"
            @input="verificaDati('via')"
            @blur="verificaDati('via')"
          />
          <Message
            v-if="errori.via.invalid"
            severity="error"
            variant="simple"
            size="small"
            >{{ errori.via.messaggio }}</Message
          >
        </div>
        ...
        <div class="flex flex-col">
          ...
          <InputText
            id="numeroCivico"
            v-model="annuncio.immobile.indirizzo.numeroCivico"
            :invalid="errori.numeroCivico.invalid"
            @input="verificaDati('numeroCivico')"
            @blur="verificaDati('numeroCivico')"
          />
          <Message
            v-if="errori.numeroCivico.invalid"
            severity="error"
            variant="simple"
            size="small"
            >{{ errori.numeroCivico.messaggio }}</Message
          >
        </div>
        ...
        <div class="flex flex-col">
          ...
          <SelectComuni
            class="w-full"
            v-model:comune="annuncio.immobile.indirizzo"
            :invalid="errori.comune.invalid"
            @blur="verificaDati('comune')"
            @change="verificaDati('comune')"
          />
          <Message
            v-if="errori.comune.invalid"
            severity="error"
            variant="simple"
            size="small"
            >{{ errori.comune.messaggio }}</Message
          >
        </div>
      </div>
...
const verificaDati = (campo) => {
  if (campo === "via") {
    ...
    const numeroCivico =
      props.annuncio.immobile.indirizzo.numeroCivico?.trim() || "";
