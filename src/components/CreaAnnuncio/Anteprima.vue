...
import {
  AnnuncioImmobiliareResponse,
  Agente,
  CaratteristicheAggiuntive,
  Contatto,
  Contratto,
  ContrattoAffittoResponse,
  ...
  Indirizzo,
  Proposta,
} from "../../dto/Response/AnnuncioImmobiliareResponse";
...
function mappaAnnuncio(input) {
  ...
  return new AnnuncioImmobiliareResponse({
    ...
    immobile: new Immobile({
      ...
      caratteristicheAggiuntive: new CaratteristicheAggiuntive(
        request.immobile.caratteristicheAggiuntive,
      ),
      immagini: request.immobile.immagini.map(
        (img) =>
          new Immagine({
