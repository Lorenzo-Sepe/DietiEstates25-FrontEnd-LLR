    ...
    <div class="form-proposta">
        <div class="card flex justify-center gap-2">
            <div class="flex-auto my-2">
                ...
                <Message severity="error" variant="simple" size="small">{{ errori.nome.messaggio }}</Message>
            </div>
            <div class="flex-auto my-2">
                <label for="Cognome" class="font-bold block mb-2"> Cognome </label>
                <InputText id="cognome" v-model="cognome" :invalid="cognomeNonValido" />
                <Message severity="error" variant="simple" size="small">{{ errori.cognome.messaggio }}</Message>
            </div>
        </div>
        <div class="flex-auto my-2">
        ...
        <div class="card flex justify-center gap-2 my-2">
            <div class="flex-auto my-2">
                ...
                <Select v-model="selectedTipoContatto" :invalid="tipoContattoNonValido" :options="tipiContatto" optionLabel="name"
                    placeholder="Seleziona tipo contatto" class="w-full md:w-56" />
                <Message severity="error" variant="simple" size="small">{{ errori.tipoContatto.messaggio }}</Message>
            </div>
            <div class="flex-auto my-2">
...
const controllaContatto = () => {
    ...
    if(contatto.value.length === 0){
        ...
        errori.contatto.invalid = true;
        errori.contatto.messaggio = 'Il contatto deve essere di massimo 30 caratteri';
        contattoNonValido.value = true;
    }else{
...
const clickNuovaProposta = async () => {
    ...
    if(errori.nome.invalid || errori.cognome.invalid || errori.prezzo.invalid || errori.contatto.invalid || errori.tipoContatto.invalid){
        return;
    }
